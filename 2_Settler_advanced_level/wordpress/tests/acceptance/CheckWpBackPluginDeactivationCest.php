...
php vendor/bin/codecept run --html --steps acceptance CheckWpBackPluginDeactivationCest


php vendor/bin/codecept run -vvv --steps acceptance CheckWpBackPluginDeactivationCest

php vendor/bin/codecept run --debug --steps acceptance CheckWpBackPluginDeactivationCest
...
class CheckWpBackPluginDeactivationCest
{
		
    public function setPluginDeactivation (AcceptanceTester $I)
    {
		...
		$I->amOnPage('/wp-admin/plugins.php');
		 	...
		 	$I->comment("\n--- DEACTIVATION PLUGINS LIST");
			$I->see(PLUGINS_ACTIVATION_DEACTIVATION_LABEL_ACTIVE);
			// Click on Inactive
			$I->click(PLUGINS_ACTIVATION_DEACTIVATION_LABEL_ACTIVE);
			$I->amOnPage('/wp-admin/plugins.php?plugin_status=active');
			// See the inactive plugins list
	        $I->expect('to see the active plugins list');
		...
		foreach ($matches as $key => $line) {
        		...
        		$I->click('//*[@id="doaction"]');
        	// Plugins active with my 2 plugins deactivated codeception_modify_header_footer installed but not active yet
				$I->lookForwardTo('to see my WP Plugins deactivated now!');
				...
				$I->comment("\n--- Check the DEACTIVATION PLUGINS LIST");
				$I->expect('to see deactivated plugins I have in my list');
				foreach (PLUGINS_INACTIVE_BULK_ACTION_CHECKLIST as $pluginTitle) {
					$I->see($pluginTitle);
